...
\begin{itemize}
	...
	\item Protocol Code：核心协议代码(一个区块中只能有0或1个)
		...
		\item Nonce：标识唯一性
		\end{itemize}
	\item Nebulas Rank Value：星云指数(计算周期为一周一次，大部分区块没有)
		\begin{itemize}
		\item Account：账号或智能合约地址
		\item NRIndex：NR值
		\item NRRank：NR排序
		\end{itemize}
\end{itemize}
...
以太坊中，交易分为两种类型：普通账户交易，智能合约交易。我们在星云链的区块中，增加新的数据类型：核心协议代码和星云指数。核心协议代码作为区块链数据的一部分存储在链上，星云链基础协议的升级，是通过链上数据的追加而实现的。星云指数存储根据NR排序算法计算出的账户NR Rank值。
